...
/** Card wrapper in feed (vertical) â Avito-style clean white cards */
export function feedListingCardClass(promo: ListingCard['promoType']): string {
  const base =
    'group flex flex-col rounded-xl bg-white overflow-hidden transition-all duration-150 hover:shadow-[0_4px_16px_rgba(0,0,0,0.1)] dark:bg-zinc-900 dark:hover:shadow-[0_4px_16px_rgba(0,0,0,0.3)]';
  if (promo === 'TOP') {
...
/** Price in card â Avito shows price prominently */
export function feedListingPriceClass(): string {
  return 'text-[15px] font-bold text-[#111] dark:text-zinc-50 mb-0.5 md:text-base';
}
