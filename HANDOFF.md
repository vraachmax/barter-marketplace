...
## 2026-09-26: PR #32 опубликован; отступы завершены, экспорт изучен

PR: https://github.com/vraachmax/barter-marketplace/pull/32
Проверенный head: 74f0b1cbd8a5a3e48ada95dcbfbd4f2db0e03894.
Merge: af8e5d46f8e430dbc7cdefb230d7d36322729d57.
Vercel production success:
https://vercel.com/vraachmaxs-projects/web/G9dp8s3reLUQX6sbmcA4TjViUCAv
Web CI 36258405614: success, 69 unit tests,build/TypeScript,6 account,
10 layout (360/440/768/820/1280, обе темы),6 messages.
Dependency audit 36258406093: success. Отдельный lint не запускался.
Artifacts: layout 10911472699, account 10911398070, messages 10911561738.
Контрольный лист снимков просмотрен, включая нижнюю часть тарифов.

Изменены 7 экранов: главная, поиск, объявления, тарифы, профиль, настройки,
кошелёк. Поля 16/24, шапки тарифов/кошелька и содержимое одной ширины,
общая строка фильтров поиска, секции тарифов 32/48, карточки 16/24.
Один нижний reserve 116+safe-area mobile / 40 desktop вместо суммы page+body;
исходная геометрия хаба сохранена. Проверена достижимость последних блоков
тарифов/кошелька выше пузыря с зазором>= 16px и breakpoint 768.
Полное основание: docs/SPACING_REVIEW.md и DESIGN_STANDARD.

Production desktop: тарифы до правок имели 200px расхождения внутренних левых
краёв шапки/контента; после оба max 1152 и padding 24. Кошелёк также совпадает.
Тарифы сначала skeleton, затем успешно загрузились до и после публикации.
Поиск до правок: фильтры от 12px края окна при центральном max 768 контенте.
Главная дважды показала timeout. Причина не установлена и не исправлена.
Авторизованные сценарии покрыты фикстурами, не реальным аккаунтом.
Физический iPhone, клавиатура и доступность из РФ без VPN не проверены.

После завершения интерфейсного этапа прочитан barter_chat_export.txt целиком.
Требования/предложения разделены в docs/PRODUCT_REQUIREMENTS_2026_09_26.md.
Приняты: единое объявление/две ленты, справочники, кросс-категорийные
последовательные обмены, аналитика/история, работа в РФ, VK/Яндекс и документы.
Мобильный стек и облачный провайдер не выбраны; старые юридические утверждения
и проценты успеха не считаются проверенными фактами. Реализация этих новых
направлений в этом блоке не начиналась.
Следующий шаг: сверка текущего Listing/API/поиска с тремя режимами и общим id.
Если воспроизводится блокирующий сбой каталога, сначала PERF-01.
UI-01/02 и отложенные интеграции не закрыты. Эта запись обновляет только docs.

## 2026-09-21: PR #31 опубликован, компоновка знака и масштаб иконки

PR: https://github.com/vraachmax/barter-marketplace/pull/31
Проверенный head: 347993feec21e5f6e5dffa3c804dcfbfef45c32b.
Merge: d4c716ccaba6dff981eed5959b8df4d774a2b427.
Vercel production: success,
https://vercel.com/vraachmaxs-projects/web/G5fEYqKS8NBqNTcE6gqWTuLY8Jj2.
Web CI 35559067539: success; 69 tests, build/TypeScript, 6 account + 8 layout +
6 messages browser runs. Audit 35559067831: success. Lint отдельно не запускался.
Просмотрены Apple PNG и CI review sheet с шапкой в обеих темах.
Artifacts (14 дней): account-browser-evidence 10620944037,
messages-browser-evidence 10620929001, listings-layout-evidence 10620479656.
Дальнейший docs-only commit фиксирует результат и не меняет код приложения.


Пользователь уточнил: речь о неполном знаке в приложении и маленькой/обрезанной
иконке при добавлении на домашний экран. Старый CSS использовал contain;
обрезка цветных пикселей в исходном PNG не подтверждена. Не приписывать устройству
установленную причину без проверки. Архивные иконки имели ~72% высоты рисунка.

Опубликована новая компоновка через статические Next ImageResponse routes:
полный оригинал, поля по измеренным границам рисунка, Apple/PWA 84% высоты,
maskable 60% с рисунком внутри круга радиуса 40%. Знак для шапки 160x192,
contain в несжимаемом боксе, ссылка по содержимому и минимум 44px.
Новые пути /brand/bubble-b-v2/; app/favicon.ico удалён из автоматических
metadata-кандидатов, явные PNG favicon v2 сохранены. Старые public PNG не удалены.
Ни перерисовки бренда, ни изменения исходных байтов: меняется компоновка оригинала.

Приёмка проверяет отдаваемые PNG, обе цветные формы, поля по всем краям,
центр и масштаб, непрозрачность app icons, safe circle maskable и геометрию шапки.
CI и публикация успешны. Физический iPhone/старый ярлык не проверены.
Следующий шаг: проверить на iPhone весь контур знака и новую иконку при добавлении.
Обновление уже установленного ярлыка не гарантируется; если замечание останется,
нужен снимок с конкретным отсутствующим фрагментом. Не объявлять исходную причину
обрезки доказанной. После этой приёмки вернуться к UI-02 browser reconnect/cookie.
